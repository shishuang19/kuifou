...
class AppDatabase extends _$AppDatabase {
  ...
  MigrationStrategy get migration => MigrationStrategy(
        onUpgrade: (migrator, from, to) async {
          if (from < 2) {
            await migrator.addColumn(categories, categories.description);
          }
        },
      );
  ...
  Future<List<AssetDb>> getAssetsByCategory(String categoryId) => (select(
          assets)
  ...
  Future<MetricsDb?> getLatestMetrics() => (select(metricsCache)
        ..orderBy([
  ...
  Future<List<MetricsDb>> getMetricsByDate(DateTime date) => (select(
          metricsCache)
